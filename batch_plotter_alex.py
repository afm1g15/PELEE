...
#-------------------------------
        ...
        nue_selected = self.samples["nue_nue"].query(query)
        ...
        nue_eff = self.plot_signal_and_eff_and_B(nue_selected_fid, self.samples["nue_nue"], nue_fiduc_q, bins, self.samples["nue_nue"].query(nue_fiduc_q))

        nue_ratio_nums = []

        for i in range(len(nue_wanted_list_smeared)):
            if (math.isnan(nue_eff[i]) == False):
                num = nue_wanted_list_smeared[i]*(1/nue_eff[i])
                nue_ratio_nums.append(num)
            else:
                nue_ratio_nums.append(0)
